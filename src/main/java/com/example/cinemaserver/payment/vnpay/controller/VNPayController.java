...
import com.example.cinemaserver.payment.vnpay.request.CreateVNPayRequest;
import com.example.cinemaserver.payment.vnpay.request.GetVNPayRequest;
import com.example.cinemaserver.payment.vnpay.request.RefundVNPayRequest;
import com.example.cinemaserver.payment.vnpay.response.CreateVNPayResponse;
import com.example.cinemaserver.payment.vnpay.response.GetVNPayResponse;
...
public class VNPayController {
    ...
    public ResponseEntity<?> createPayment(HttpServletRequest req, @ModelAttribute CreateVNPayRequest createPaymentRequest) {
        try{
            ...
            CreateVNPayResponse createPaymentResponse=new CreateVNPayResponse();
    ...
    public ResponseEntity<?> getPayment(HttpServletRequest req, @ModelAttribute GetVNPayRequest getPaymentRequest){
        try{
            ...
            String hash_Data= String.join("|", vnp_RequestId, vnp_Version, vnp_Command
                    , vnp_TmnCode, vnp_TxnRef, vnp_TransactionDate, vnp_CreateDate, vnp_IpAddr, vnp_OrderInfo);
            String vnp_SecureHash = VNPayConfig.hmacSHA512(VNPayConfig.secretKey, hash_Data.toString());
            ...
            return ResponseEntity.ok(objectMapper.convertValue(map, GetVNPayResponse.class));
        }catch (Exception e){
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error getting payment");
        }
    }
}
