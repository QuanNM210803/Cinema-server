...
public interface BillRepository extends JpaRepository<Bill,Long> {
    ...
    @Query("SELECT b FROM Bill b WHERE b.user.id=:userId ORDER BY b.createdTime DESC ")
    List<Bill> findByUserId(Long userId);

    @Query("SELECT b FROM Bill b WHERE DATE (b.createdTime)=:currentDate " +
            "AND b.id IN (SELECT DISTINCT (t.bill.id) FROM Ticket t WHERE t.schedule.id IN " +
